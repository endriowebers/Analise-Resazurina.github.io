...
```{r}
fluorescencia_data_2 = read_excel("colle 2 placa.xlsx",
                      sheet = "Fluorescencia")

head(fluorescencia_data_2)

abs_data_2 = read_excel("colle 2 placa.xlsx",
                      sheet = "Absorbancia")
...
```{r}
fluo_completo_2 = fluorescencia_data_2 |> 
...
```{r}
abs_completo_2 %>% 
  # Etapa 1: Filtrar e sumarizar os dados
  filter(
    tratamento == "fungo",
    concentracao == 1e+05,
    meio == "YB"
  ) %>% 
  group_by(leitura, comprimento_onda) %>% 
  summarise(mean_abs = mean(abs), .groups = "drop") %>%
  
  # Etapa 2: Pivotar os dados para o formato de matriz (largo)
  pivot_wider(
    names_from = leitura,
    values_from = mean_abs,
    id_cols = comprimento_onda
  ) %>%
  
  # Etapa 3: Bloco final que agora funciona com o pipe %>%
  {
    x_axis <- as.numeric(colnames(.)[-1])
    y_axis <- .[[1]]
    z_matrix <- as.matrix(.[,-1])
    ...
    plot_ly(
      ...
      layout(
        title = "Superfície: Fungo, Conc: 100.000, Meio: YB",
        scene = list(
          xaxis = list(title = "Leitura"),
          yaxis = list(title = "Comprimento de Onda (nm)"),
          zaxis = list(title = "Absorbância Média")
        )
      )
  }
```

# multiplos gráficos de superfície resposta

```{r}
#| label: plotar-todos-os-graficos
#| fig-cap: "Todos os gráficos de superfície de resposta combinados."


# --- 2. FUNÇÃO CORRIGIDA ---
# Corrigimos a linha 'format()' para ser mais explícita e remover o aviso.
plotar_superficie_3d <- function(dados, trat_selecionado, conc_selecionada, meio_selecionado) {
  dados_filtrados <- dados %>%
    filter(tratamento == trat_selecionado, concentracao == conc_selecionada, meio == meio_selecionado)
  
  if (nrow(dados_filtrados) == 0) {
    # Retorna NULL se não houver dados, para não quebrar o pmap
    return(NULL)
  }
  
  # CORREÇÃO: Adicionado decimal.mark = "," para resolver o aviso
  titulo_grafico <- paste0(
    "Superfície: ", trat_selecionado, 
    " | Conc: ", format(conc_selecionada, scientific = FALSE, big.mark = ".", decimal.mark = ","), 
    " | Meio: ", meio_selecionado
  )
  
  dados_filtrados %>%
    group_by(leitura, comprimento_onda) %>%
    summarise(mean_abs = mean(abs), .groups = "drop") %>%
    pivot_wider(names_from = leitura, values_from = mean_abs, id_cols = comprimento_onda) %>%
    {
      x_axis <- as.numeric(colnames(.)[-1])
      y_axis <- .[[1]]
      z_matrix <- as.matrix(.[,-1])
      
      plot_ly(x = ~x_axis, y = ~y_axis, z = ~z_matrix, type = "surface", showscale = FALSE) %>%
        layout(
          title = list(text = titulo_grafico, y = 0.98, font = list(size = 10)),
          scene = list(
            xaxis = list(title = "Leitura", tickfont = list(size=8)),
            yaxis = list(title = "Comp. Onda (nm)", tickfont = list(size=8)),
            zaxis = list(title = "Abs Média", tickfont = list(size=8))
          )
        )
    }
}

# --- 3. CRIAR A LISTA DE GRÁFICOS (como você já fez) ---
combinacoes <- abs_completo %>% distinct(tratamento, concentracao, meio)

lista_de_todos_os_plots_2 <- pmap(list(
  trat_selecionado = combinacoes$tratamento,
  conc_selecionada = combinacoes$concentracao,
  meio_selecionado = combinacoes$meio
), .f = ~plotar_superficie_3d(
  dados = abs_completo,
  trat_selecionado = ..1,
  conc_selecionada = ..2,
  meio_selecionado = ..3
...
# Remove quaisquer resultados nulos (casos onde não havia dados)
lista_de_todos_os_plots_2 <- compact(lista_de_todos_os_plots)

# --- 4. MOSTRAR OS GRÁFICOS! ---
# Agora usamos subplot() para combinar e exibir todos os gráficos da lista
if (length(lista_de_todos_os_plots_2) > 0) {
  subplot(lista_de_todos_os_plots_2, nrows = ceiling(length(lista_de_todos_os_plots_2) / 3)) %>%
    layout(title = "<b>Visualização de Todas as Combinações</b>")
} else {
...
```{r}
lista_de_todos_os_plots_2[[1]]
lista_de_todos_os_plots_2[[2]]
lista_de_todos_os_plots_2[[3]]
```
...
```{r}
resultados_finais_2 <- calcular_reducao_resazurina(
  dados = abs_completo_2,
